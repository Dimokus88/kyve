...
>Сейчас доступно 2 пула, контейнер настроен таким образом, что если у вас нехватает депозита для стейка в пуле 0, то нода переключится на пул 1 и наоборот. Таким образом, нода будет перебирать пулы пока не подключится. Попытки подключения делаются раз в 5 минут. Вся информация будет выводится во вкладку LOGS.  
